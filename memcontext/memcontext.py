...
try:
    ...
    from .mid_term import MidTermMemory, compute_segment_heat
...
class Memcontext:
    def __init__(self, user_id: str, 
        ...
        if file_storage_manager is None:
            try:
                from memcontext.file_storage import FileStorageManager
                ...
                print(f"FileStorageManager files_dir will be: {self.file_storage_manager.files_dir}")
            except ImportError as e:
                print(f"Warning: file_storage module not found, file storage features will be disabled. Error: {e}")
                import traceback
                traceback.print_exc()
                self.file_storage_manager = None
            except Exception as e:
                print(f"Warning: Failed to initialize FileStorageManager: {e}")
                import traceback
                traceback.print_exc()
                self.file_storage_manager = None
        else:
...
# 在 Memcontext 类中添加此新方法
    ...
    def _ingest_single_multimodal(
        ...
        *,
        ...
        # videorag converter 现在支持可选的文件管理（和 video_converter 一样）
            ...
            try:
                ...
                if file_extension in ['mp4', 'mov', 'avi', 'mkv', 'webm', 'flv', 'm4v']:
                    ...
                    if not stored_file_id:
                        print(f"FileStorageManager: Uploading video file from {file_path_str}")
                        print(f"FileStorageManager: storage_base_path={self.file_storage_manager.storage_base_path}")
                        ...
                        print(f"FileStorageManager: Stored path={stored_file_path}")
                        # 验证文件是否真的存在
                        if not os.path.exists(stored_file_path):
                            raise FileNotFoundError(f"FileStorageManager: File uploaded but not found at {stored_file_path}")
                        print(f"FileStorageManager: File verified at {stored_file_path}")
        ...
        # 返回结果：优先返回 file_storage_id（如果存在），否则返回 source_file_id
        result_file_id = stored_file_id if stored_file_id else (base_metadata.get("file_storage_id") or base_metadata.get("source_file_id"))
        result = {
        ...
        # 如果有 file_storage_manager，添加存储路径信息
        if stored_file_id and self.file_storage_manager:
            storage_path = os.path.join(
                self.file_storage_manager.storage_base_path,
                "files", "videos", stored_file_id
            )
            result["storage_path"] = storage_path
            result["storage_base_path"] = self.file_storage_manager.storage_base_path
            # 验证文件是否真的存在
            if stored_file_path and os.path.exists(stored_file_path):
                result["file_exists"] = True
                result["actual_file_path"] = stored_file_path
            else:
                result["file_exists"] = False
                print(f"Warning: FileStorageManager says file stored at {stored_file_path}, but file does not exist!")
        return result
    ...
    def _extract_filename_from_query(self, query: str) -> Optional[str]:
        ...
        Returns:
            提取到的文件名，如果没有找到则返回None
        """
        import re
        
        # 匹配常见的文件名查询模式
        patterns = [
            r'文件[名称为叫]是[：:]\s*([^\s，,。.！!？?\n]+\.(mp4|mov|avi|mkv|webm|flv|m4v|jpg|jpeg|png|gif|pdf|doc|docx|txt|mp3|wav))',
            r'文件名[：:]\s*([^\s，,。.！!？?\n]+\.(mp4|mov|avi|mkv|webm|flv|m4v|jpg|jpeg|png|gif|pdf|doc|docx|txt|mp3|wav))',
            r'查找[文件]?[：:]\s*([^\s，,。.！!？?\n]+\.(mp4|mov|avi|mkv|webm|flv|m4v|jpg|jpeg|png|gif|pdf|doc|docx|txt|mp3|wav))',
            r'([^\s，,。.！!？?\n]+\.(mp4|mov|avi|mkv|webm|flv|m4v|jpg|jpeg|png|gif|pdf|doc|docx|txt|mp3|wav))\s*这个文件',
            r'([^\s，,。.！!？?\n]+\.(mp4|mov|avi|mkv|webm|flv|m4v|jpg|jpeg|png|gif|pdf|doc|docx|txt|mp3|wav))\s*在哪',
            r'([^\s，,。.！!？?\n]+\.(mp4|mov|avi|mkv|webm|flv|m4v|jpg|jpeg|png|gif|pdf|doc|docx|txt|mp3|wav))\s*的路径',
        ]
