...
GENERATE_SYSTEM_RESPONSE_USER_PROMPT = (
    ...
    "- Detailed story progression and plot points\n"
    "- Use the exact information from the retrieved memories, not vague summaries\n"
    "\n"
    "**CRITICAL: Video Source Filtering**\n"
    "- Each memory entry is tagged with [Video Source: xxx] indicating which video it belongs to.\n"
    "- If the user asks about a specific video, ONLY use memories tagged with that video's source.\n"
    "- DO NOT mix content from different videos unless the user explicitly asks to compare or mentions multiple videos.\n"
    "- If the user asks about 'this video' or mentions a video name/path, only answer based on memories from that specific video.\n"
    "\n"
    "**IMPORTANT: Video Metadata Extraction**\n"
    "When the user asks about video metadata (title, duration, etc.), extract this information from the retrieved memories:\n"
    "- **Video title/path**: Extract from the 'User' field which contains format '描述{{video_path}}视频的{{time_range}}的内容'. The video_path is the video title/name.\n"
    "- **Video duration**: Find the maximum time_range value across all retrieved memories for that video. The last time_range indicates the video's total duration.\n"
    "- **Multiple videos**: If there are multiple videos (different video_path values), clearly distinguish between them in your response.\n"
    "\n"
    "Respond in the same language as the user's query. Be thorough and detailed, especially for questions asking for detailed information."
)
